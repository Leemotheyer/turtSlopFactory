...
async def _post_production_scheduler_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.exception("Post-production scheduler failed")
...
async def lifespan(app: FastAPI):
    ...
    post_prod_task = asyncio.create_task(_post_production_scheduler_loop())
    worker_task = None
    ...
    try:
        ...
        redis_task.cancel()
        expire_task.cancel()
        post_prod_task.cancel()
        if worker_task:
            worker_task.cancel()
        with contextlib.suppress(asyncio.CancelledError):
            await redis_task
            await expire_task
            await post_prod_task
            if worker_task:
                await worker_task
