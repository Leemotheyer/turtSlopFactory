    ...
    async def run_pipeline(self, project_id: UUID) -> None:
        ...
        try:
            async with SessionLocal() as session:
                ...
                from app.services.self_propelling import is_self_propelling_enabled

                meta = self.workspace.load_metadata(project_id)
                if ProjectState(project.state) == ProjectState.PRODUCTION and (
                    meta.get("post_production_pending")
                    or is_self_propelling_enabled(project_id, self.workspace)
                ):
    ...
    async def _run_enrichment_passes(
        ...
        *,
        ...
        while passes_done < max_passes:
            ...
            skip_architect = should_skip_architect(project.id, audit, self.workspace)
            plan_raw = None
            used_fallback = False
            run = None
            if skip_architect:
            ...
            test_ok, test_output = await self.test_runner.run_unit(project.id, context)
            ...
            mobile_task = await self.create_task(
                session,
                project.id,
                f"Mobile check (pass {pass_number})",
                "Verify mobile-friendly layout signals",
                AgentRole.TESTER,
            )
            ...
            context["mobile_check"] = mobile_output
