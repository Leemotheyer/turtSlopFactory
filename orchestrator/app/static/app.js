...
function renderMain() {
  ...
  } else if (d.state === "AUTONOMOUSLY_BLOCKED") {
    actions = '<button type="button" class="btn btn-primary" id="btn-run">Resume pipeline</button>';
  } else {
  ...
  el.innerHTML = `
    <div class="project-top">
      ...
      <div class="actions">${actions}<button type="button" class="btn btn-ghost" id="btn-delete">Delete</button></div>
    </div>
    ${previewHtml}
...
async function deleteProject() {
  ...
  if (!confirm(`Delete "${state.detail.name}"? This cannot be undone.`)) return;
  state.loading = true;
  try {
    await api(`/projects/${state.selectedId}`, { method: "DELETE" });
