...
def test_default_parallel_streams():
    ...
    assert streams == ["backend", "frontend", "feature", "feature", "feature", "feature"]
    ...
    assert "search-settings" in feature_ids
    assert "domain-features" in feature_ids
...
def test_feature_notes_spawn_parallel_agents():
    ...
    assert streams.count("feature") == 6
...
def test_multi_line_feature_note_splits_into_units():
    notes = [
        {
            "type": "feature",
    ...
    units = plan_parallel_work(notes, "App")
    feature_units = [u for u in units if u.stream == "feature" and u.feature_id not in {
        "ux-polish", "core-completeness", "search-settings", "domain-features"
    }]
    assert len(feature_units) == 3
